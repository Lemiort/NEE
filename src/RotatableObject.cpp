#include "RotatableObject.h"

RotatableObject::RotatableObject() {
    // ctor
    model_rotation[0] = 0.0f;
    model_rotation[1] = 0.0f;
    model_rotation[2] = 0.0f;
}

RotatableObject::~RotatableObject() {
    // dtor
}

void RotatableObject::SetRotation(float x, float y, float z) {
    model_rotation[0] = x;
    model_rotation[1] = y;
    model_rotation[2] = z;
}

void RotatableObject::SetRotation(glm::vec3 rot) {
    model_rotation[0] = rot.x;
    model_rotation[1] = rot.y;
    model_rotation[2] = rot.z;
}
glm::vec3 RotatableObject::GetRotation() {
    return glm::vec3(model_rotation[0], model_rotation[1], model_rotation[2]);
}
