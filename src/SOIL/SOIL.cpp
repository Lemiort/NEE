...
unsigned int SOIL_load_OGL_cubemap(
    ...
    img =
        SOIL_load_image(x_pos_file, &width, &height, &channels, force_channels);
    /*	channels holds the original number of channels, which may have been
     * forced	*/
    if ((force_channels >= 1) && (force_channels <= 4)) {
        channels = force_channels;
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image(x_neg_file, &width, &height, &channels,
                              force_channels);
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image(y_pos_file, &width, &height, &channels,
                              force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
        if ((force_channels >= 1) && (force_channels <= 4)) {
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image(y_neg_file, &width, &height, &channels,
                              force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
        if ((force_channels >= 1) && (force_channels <= 4)) {
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image(z_pos_file, &width, &height, &channels,
                              force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
        if ((force_channels >= 1) && (force_channels <= 4)) {
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image(z_neg_file, &width, &height, &channels,
                              force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
        if ((force_channels >= 1) && (force_channels <= 4)) {
...
unsigned int SOIL_load_OGL_cubemap_from_memory(
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image_from_memory(x_neg_buffer, x_neg_buffer_length,
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image_from_memory(y_pos_buffer, y_pos_buffer_length,
                                          &width, &height, &channels,
                                          force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image_from_memory(y_neg_buffer, y_neg_buffer_length,
                                          &width, &height, &channels,
                                          force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image_from_memory(z_pos_buffer, z_pos_buffer_length,
                                          &width, &height, &channels,
                                          force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
    ...
    if (tex_id != 0) {
        /*	1st face: try to load the image	*/
        img = SOIL_load_image_from_memory(z_neg_buffer, z_neg_buffer_length,
                                          &width, &height, &channels,
                                          force_channels);
        /*	channels holds the original number of channels, which may have
         * been forced	*/
...
unsigned int SOIL_load_OGL_single_cubemap(const char *filename,
    ...
    tex_id = SOIL_create_OGL_single_cubemap(
        img, width, height, channels, face_order, reuse_texture_ID, flags);
...
unsigned int SOIL_internal_create_OGL_texture(
    ...
    if ((flags & SOIL_FLAG_POWER_OF_TWO) || /*	user asked for it	*/
        (flags &
         SOIL_FLAG_MIPMAPS) || /*	need it for the MIP-maps	*/
        (width >
...
int query_DXT_capability(void) {
    ...
    if (has_DXT_capability == SOIL_CAPABILITY_UNKNOWN) {
        ...
        if (NULL == strstr((char const *)glGetString(GL_EXTENSIONS),
            ...
            CFRelease(bundle);
#else
            ext_addr = (P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC)glfwGetProcAddress(
                "glCompressedTexImage2DARB");
#endif
