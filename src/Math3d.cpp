...
void Vector3f::Rotate(float Angle, const Vector3f& Axe) {
    ...
    Quaternion const RotationQ(Rx, Ry, Rz, Rw);

    Quaternion const ConjugateQ = RotationQ.Conjugate();
    //  ConjugateQ.Normalize();
    Quaternion const W = RotationQ * (*this) * ConjugateQ;
...
float Vector3f::Lenght() const { return sqrtf((x * x) + (y * y) + (z * z)); }
...
void Quaternion::Normalize() {
    float const Length = sqrtf((x * x) + (y * y) + (z * z) + (w * w));
